...
export class FollowsService {
  ...
  async getNumOfFollowersFollowing(userId: number) {
    const numbersOfFollowers = await this.followsRepository.find({
      where: { followingId: userId },
    });
    const numbersOfFollowing = await this.followsRepository.find({
      where: { followerId: userId },
    });
    return {
      numFollowers: numbersOfFollowers.length,
      numFollowing: numbersOfFollowing.length,
    };
  }
